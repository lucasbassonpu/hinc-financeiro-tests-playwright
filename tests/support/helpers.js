...
import {
  moneyStringToNumber,
  percentageStringToNumber,
  flexibleNumberToNumber,
} from "./formatters.js";
...
export async function readMoney(page, selector) {
  if (!selector || selector === undefined) {
    throw new Error(
      `Selector is undefined or null. Check elements.js for missing selectors.`,
    );
  }
...
export async function getChartCanvas(
  page,
  containerSelector = ".ag-chart-wrapper",
) {
  return page.locator(containerSelector).locator("canvas");
}

// Interact with AG-Charts tooltips
export async function hoverChartPoint(
  page,
  x,
  y,
  containerSelector = ".ag-chart-wrapper",
...
export async function clickChartLegendItem(page, legendText) {
  await page
    .locator(".ag-chart-legend-item")
    .filter({ hasText: legendText })
    .click();
}
...
export function expectCloseTo(actual, expected, delta = 0.005) {
  ...
  if (diff > delta) {
    throw new Error(
      `Expected ${actual} to be close to ${expected} (within ${delta}), but difference was ${diff}`,
    );
  }
}
